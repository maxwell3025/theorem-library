...
class Project(neomodel.StructuredNode):
    ...
    has_valid_dependencies = neomodel.StringProperty(
        choices=ternary_choices, default="unknown"
    )
    has_valid_proof = neomodel.StringProperty(
        choices=ternary_choices, default="unknown"
    )
    has_valid_paper = neomodel.StringProperty(
        choices=ternary_choices, default="unknown"
    )

    dependencies = neomodel.RelationshipTo("Project", "DEPENDS_ON")
